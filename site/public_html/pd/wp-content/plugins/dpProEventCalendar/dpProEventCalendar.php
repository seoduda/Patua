<?php
/*
Plugin Name: DP Pro Event Calendar - kingtheme.net
...
define("DP_PRO_EVENT_CALENDAR_VER","2.4.6",false);//Current Version of this plugin
...
function install_dpProEventCalendar() {
	...
	if($wpdb->get_var("show tables like '$table_name_calendars'") != $table_name_calendars) {
		$sql = "CREATE TABLE $table_name_calendars (
					...
					lang_txt_weekly VARCHAR(80) NOT NULL DEFAULT 'Weekly',
					...
					lang_tickets_remaining VARCHAR(80) NOT NULL DEFAULT 'Tickets Remaining',
					lang_next_day VARCHAR(80) NOT NULL DEFAULT 'Next Day',
					lang_prev_day VARCHAR(80) NOT NULL DEFAULT 'Prev Day',
					lang_next_week VARCHAR(80) NOT NULL DEFAULT 'Next Week',
					lang_prev_week VARCHAR(80) NOT NULL DEFAULT 'Prev Week',
...
if(!isset($dpProEventCalendar['tickets_remaining'])) {
	$dpProEventCalendar['tickets_remaining'] = true;
	
	$sql = "ALTER TABLE $table_name_calendars ADD (lang_tickets_remaining VARCHAR(80) NOT NULL DEFAULT 'Tickets Remaining');";
	$wpdb->query($sql);
	
	update_option('dpProEventCalendar_options',$dpProEventCalendar);
}

if(!isset($dpProEventCalendar['weekly_view'])) {
	$dpProEventCalendar['weekly_view'] = true;
	
	$sql = "ALTER TABLE $table_name_calendars ADD (lang_next_week VARCHAR(80) NOT NULL DEFAULT 'Next Week');";
	$wpdb->query($sql);
	
	$sql = "ALTER TABLE $table_name_calendars ADD (lang_prev_week VARCHAR(80) NOT NULL DEFAULT 'Prev Week');";
	$wpdb->query($sql);
	
	$sql = "ALTER TABLE $table_name_calendars ADD (lang_txt_weekly VARCHAR(80) NOT NULL DEFAULT 'Weekly');";
	$wpdb->query($sql);
