	...
	if ( !empty( $mails ) ) {
		foreach ( $mails as $mail ) {
			if ( $mail->id == $email_type ) {
				...
				// Get the Customer user_id fromt he order, or the current user ID if guest.
				...
				<div class="email-template-preview <?php echo $presentation_state ?> ">
					<div class="main-content">
						...
						<?php if ( $compatabiltiy_warning && ( $mail->id !== $_REQUEST['ec_approve_preview'] ) ) : ?>
						...
						<!-- ----------  Email Template ---------- -->
						<?php
						@ $email_render = do_shortcode( $mail->style_inline( $mail->get_content() ) );
						echo $email_render;
						?>
						<!-- ----------  /Email Template ---------- -->
