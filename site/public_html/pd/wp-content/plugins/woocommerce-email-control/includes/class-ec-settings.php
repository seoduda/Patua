...
class EC_Settings {
										...
										<div class="controls-field">
											<div class="controls-inner-row">
												...
												<?php if ($value['default']): ?>
													<span class="reset-to-default help_tip_new" data-tip="<?php echo esc_attr( __("Reset to - ",'email-control') . ' ' . $value['default'] ); ?>" data-default="<?php echo esc_html( $value['default'] ); ?>">
														Reset to default <i class="fa fa-refresh"></i>
													</span>
												<?php endif ?>
	...
	public static function ec_render_option( $option_name, $option_value = '' ) {
		...
		return __( $option_value, 'email-control' );
	}
	...
	public static function ec_normalize_template_args() {
		...
		$defaults = array(
			...
			'email'         => ( isset( $_REQUEST["ec_render_email"] ) ) ? 'email@email.com' : '' ,
		...
		elseif ( isset( $ec_template_args['user_login'] ) ) {
			...
			if ( ( $first_name = get_user_meta( $user->ID, 'first_name', true ) ) )
			...
			if ( ( $last_name = get_user_meta( $user->ID, 'last_name', true ) ) )
	...
	public static function ec_customer_note( $shortcode_args ) {
		...
		elseif ( isset( $_REQUEST["ec_email_template"] ) ) {
			$content = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas a nisi in dui consectetur ultricies eu id tellus. Sed elementum, metus vitae volutpat hendrerit, mi est rutrum enim, eget sodales nulla risus sit amet justo. Mauris luctus eu arcu eget malesuada.';
		}
